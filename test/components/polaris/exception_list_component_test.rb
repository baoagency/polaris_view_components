...
class ExceptionListComponentTest < Minitest::Test
  ...
  def test_exception_list
    render_inline(Polaris::ExceptionListComponent.new) do |list|
      list.item(icon: "NoteMinor", title: "Title 1") { "Default Note" }
      list.item(icon: "NoteMinor", title: "Title 2", status: :warning) { "Warning Note" }
      list.item(icon: "NoteMinor", title: "Title 3", status: :critical) { "Critical Note" }
    end
